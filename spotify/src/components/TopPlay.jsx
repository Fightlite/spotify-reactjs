...
const TopChartCard = ({ song, index, isPlaying, activeSong, handlePauseClick, handlePlayClick }) => (
  <div className="w-full flex flex-row items-center hover:bg-[#4c426e] py-2 p-4 rounded-lg cursor-pointer mb-2">
    <h3 className="text-base font-bold text-white mr-3">
      {index + 1}.
    </h3>
    <div className="flex flex-1 flex-row justify-between items-center">
      <img src={song?.images?.coverart} alt={song?.title} className="rounded-lg w-20 h-20 object-contain" />
      ...
      <div className="w-full flex flex-col mt-8">
        ...
        >
          {topPlays?.map((song, index) => (
            ...
            >
              <Link to={`/artists/${song?.artists[0].adamid}`} >
                <img src={song?.images.background} alt="" className="rounded-full w-full object-contain" />
              </Link>
            </SwiperSlide>
          ))}
        </Swiper>
